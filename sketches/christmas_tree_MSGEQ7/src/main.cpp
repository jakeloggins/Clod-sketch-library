...
// -- MSGEQ7
  ...
  #define PIN_SOUND A0 // -- pretty sure there's only one pin for the esp8266
  ...
  {
   return map(powerValue, MINPOWER, MAXPOWER, 0, PixelCount);
  }
...
void loop() {
  ...
  if (musicOverride) {
   ...
   int rColor = colorValue((soundvalue[5]+soundvalue[6])/2);
    ...
    for (int i = 0; i < PixelCount; i++) {
      ...
      if(i<=240) {
        strip.SetPixelColor(i, (i<=rColor) ? rColor : 0,(i<=gColor) ? gColor : 0,(i<=bColor) ? bColor : 0);
      }
      else {
        strip.SetPixelColor(i, (i<=rColor) ? 255 : 0,(i<=gColor) ? 255 : 0,(i<=bColor) ? 255 : 0);
      } 
    } 
    strip.Show();
  ...
  if (isAnimating == false) {
    ...
    if ((millis() - lastMQTT > timeoutSeconds) && (timeoutPlay) && (!musicOverride)) { // 30 minute timer before animations start
